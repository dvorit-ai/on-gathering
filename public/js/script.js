...
socket.on("count", function (num) {
  ...
  //set the body ID to the number of active users any more 10 than 10, it just sits at 10.
  (num <=10) ? document.body.id = "user" + num : document.body.id = "user10";
  ...
  // make footer change background color at 10+ users
  document.querySelectorAll("footer").forEach((element) => {
    if (num >= 10) {
      element.classList.add("user-10");
    } else {
      element.classList.remove("user-10");
    }
  });
